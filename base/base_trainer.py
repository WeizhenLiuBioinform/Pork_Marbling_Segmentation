...
class BaseTrainer:
    def __init__(self, model, loss, resume, config, train_loader, val_loader=None, test_loader=None, train_logger=None):
        ...
        self.device, availble_gpus = self._get_available_devices(self.config['n_gpu'], default_device)
        ...
        self.lr_scheduler = getattr(utils.lr_scheduler, config['lr_scheduler']['type'])(self.optimizer, self.epochs, len(train_loader), **config['lr_scheduler']['args'])
    ...
    def _get_available_devices(self, n_gpu, default_device):
        ...
        device = torch.device(default_device if n_gpu > 0 else 'cpu')
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            if self.do_validation and epoch % self.config['trainer']['val_per_epochs'] == 0:
                ...
                # LOGGING INFO
                self.logger.info(f'\n         ## Info for epoch {epoch} validation ## ')
                for k, v in results.items():
                    self.logger.info(f'         {str(k):15s}: {v}')
            
            if self.do_test and epoch % self.config['trainer']['test_per_epochs'] == 0:
                results = self._test_epoch(epoch)

                # LOGGING INFO
                self.logger.info(f'\n         ## Info for epoch {epoch} test ## ')
                for k, v in results.items():
                    self.logger.info(f'         {str(k):15s}: {v}')
