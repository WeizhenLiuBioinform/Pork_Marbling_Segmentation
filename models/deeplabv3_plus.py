...
from .danet import PositionAttentionModule, ChannelAttentionModule
from .ccnet import CCAttention
...
class DualAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.PAM = PositionAttentionModule(out_channels)
...
class ResNet(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        ...
        self.expansion = model.expansion
...
class DeepLab(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='xception', pretrained=True, 
                output_stride=16, freeze_bn=False, freeze_backbone=False, use_attention=True, **_):
        ...
        if 'resnet' in backbone:
            ...
            expansion = self.backbone.expansion
            low_level_channels = 64*expansion
        else:
            self.backbone = Xception(output_stride=output_stride, pretrained=pretrained)
            low_level_channels = 128
            expansion = 4

        self.ASPP = ASPP(in_channels=512*expansion, output_stride=output_stride)
        self.decoder = Decoder(low_level_channels, num_classes)

        # 注意力模块输入通道要注意
        self.attention_block = nn.Sequential()
        if use_attention:
            # self.attention_block = DualAttentionBlock(2048, low_level_channels) # 并联结构，输入输出通道与ASPP保持一致
            self.attention_block = CCAttention(512*expansion, low_level_channels)
        
        self.fusion = nn.Conv2d(low_level_channels + 256, 256, 1)
    ...
    def forward(self, x):
        ...
        if self.use_attention:
            x_aspp = self.ASPP(x)
            x_att = self.attention_block(x)
            x = torch.cat([x_aspp, x_att], dim=1)
            x = self.fusion(x)
        else:
            x = self.ASPP(x)
    ...
    def get_decoder_params(self):
        if self.use_attention:
            return chain(self.ASPP.parameters(), self.attention_block.parameters(), self.fusion.parameters(), self.decoder.parameters())
