...
"""
@author https://github.com/songqi-github/AttaNet/blob/main/AttaNet.py
"""
import torch
...
class StripAttentionModule(nn.Module):
    """
    竖直条纹注意力机制，使用条纹池化来减少参数量，加强竖直方向的特征注意力
    使用竖直的池化得到每列的均值，然后求亲和度时相当于求每个位置的像素与每列的相关性
    若某位置像素与
    """
    def __init__(self, in_chan, out_chan, *args, **kwargs):
        ...
        self.conv3 = ConvBNReLU(in_chan, out_chan, ks=1, stride=1, padding=0) # 调整维度
    ...
    def forward(self, x):
        ...
        q = F.avg_pool2d(q, [h, 1]) # 条纹池化，竖直方向 W*H ==> W*1
        ...
        v = self.conv3(x)
        c_out = v.size()[1]
        v = F.avg_pool2d(v, [h, 1]) # 条纹池化，竖直方向
        v = v.view(batchsize, c_out, -1)

        augmented_feature_map = torch.bmm(v, attention_map) # 矩阵乘法
...
class AttentionFusionModule(nn.Module):
    """
    类似GatedFeatureFusion模块使用门控机制进行特征融合，相比GFF更加简洁
    """
    def __init__(self, in_chan, out_chan, *args, **kwargs):
    ...
    def forward(self, feat16, feat32):
        feat32_up = F.interpolate(feat32, feat16.size()[2:], mode='nearest') # 最近邻池化??
